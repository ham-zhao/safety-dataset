...
**问题**：TikTok 日活超 10 亿，UGC 内容审核需要自动化安全分类能力，但公开安全数据集格式各异、质量参差、类别不均衡，且多模态攻击（图文组合绕过文本安全检查）缺乏系统性防御数据。

**方案**：整合 8 个公开安全数据集，经 Data-Juicer 风格 5 步清洗 + 5 种增强策略，训练级联分类器（DistilBERT 初筛 + CLIP 多模态精筛），在 4 个标准 Benchmark 上评估，并通过 6 组消融实验量化每个数据组件的贡献。

**预期指标**：文本分类器 AUC ≥ 0.95、Recall ≥ 0.90；多模态分类器 AUC ≥ 0.90；Over-refusal Rate < 10%。
